...
def pers_landscapes(state, complex, sc):
    ...
    plt.legend()
    plt.title(state + 'Landscapes (dimension 0, '+ sc +')')
    land0.savefig('output/pl_whole/'+ state + 'dim0'+ sc +'_pl.jpeg', bbox_inches = 'tight')
    plt.close()
    ...
    plt.plot(L[0][900:1000], label = "Landscape 10")
    plt.legend()
    plt.title(state + 'Landscapes (dimension 1, '+sc+ ')')
    land1.savefig('output/pl_whole/'+ state + 'dim1'+ sc+'_pl.jpeg', bbox_inches = 'tight')
    plt.close()
    return landscapes

def plot_landscapes(L, dim, sc):
    land = plt.figure()
    land = plt.figure()
    plt.plot(L[0][:100], label = "Landscape 1" )
    plt.plot(L[0][100:200], label = "Landscape 2")
    ...
    plt.plot(L[0][900:1000], label = "Landscape 10")
    plt.legend()
    plt.title('Landscapes (dimension ' + str(dim) +', '+sc+ ')')
    land.savefig('output/avg_pl/'+ sc+'dim' + str(dim) + '_pl.jpeg', bbox_inches = 'tight')
    plt.close()